...
Currently only displays numbers. Also only supports common cathode.
Will create a common anode version in a future version
"""

  def __init__(self, A=0, B=1, C=2, D=3, E=4, F=5, G=6, *, dataPin=None, clockPin=None, latchPin=None):
  """
  ...
  def _parallel_update(self, input):
    for i in range(0,7):
      self._parallelPins[i].value(int(input[i])) 
